...
type TestWriter struct {
	...
	schemas    map[string]*awseventgenerator.Schema
...
func NewTestWriter() *TestWriter {
	return &TestWriter{
		...
		schemas:    make(map[string]*awseventgenerator.Schema),
		imports:    make(map[string]struct{}),
	}
}

func (tw *TestWriter) Add(data []byte, label, pkgName, folderName string, schema *awseventgenerator.Schema) error {
	...
	tw.labels[pkgName] = label
	tw.folders[pkgName] = folderName
	tw.schemas[pkgName] = schema
...
func (tw *TestWriter) Generate() ([]byte, error) {
	...
	// tw.imports["os"] = struct{}{}
	// tw.imports["github.com/jmespath/go-jmespath"] = struct{}{}
	tw.imports["github.com/stretchr/testify/require"] = struct{}{}
	tw.imports["github.com/webdestroya/awseventgenerator/internal/testutil/genhelpers"] = struct{}{}
	// tw.imports["github.com/webdestroya/awseventgenerator/internal/testutil/astinspector"] = struct{}{}

	// hacky, but since we only ever deal with 1 package, we're ok
	lastPackage := ""

	for pkgName := range tw.inspectors {
		lastPackage = pkgName
		fmt.Fprintf(&buf, `func TestGenerated_%s_%s(t *testing.T) {`+"\n", pkgName, tw.labels[pkgName])
		fmt.Fprintln(&buf)
	...
	fmt.Fprintf(&finalBuf, `package %s_test`+"\n", lastPackage)
	fmt.Fprintln(&finalBuf)
...
func (tw *TestWriter) genStructMarshalTests(buf io.Writer, ins *astinspector.Inspector, pkgName, structName string, strct *ast.StructType) {
	...
	for _, f := range strct.Fields.List {
		...
		switch ftypeStr {
		case "*string", "string":
			fmt.Fprintf(buf, "genhelpers.RequireJmesMatch(t, jsearch, `\"%s\"`, strVal, \"%s\")\n", tagKey, fname)
		case "[]string":
			fmt.Fprintf(buf, "genhelpers.RequireJmesMatch(t, jsearch, `\"%s\"[0]`, strVal, \"%s\")\n", tagKey, fname)
		case "*float64", "float64":
			fmt.Fprintf(buf, "genhelpers.RequireJmesMatch(t, jsearch, `\"%s\"`, floatVal, \"%s\")\n", tagKey, fname)
		case "[]float64":
			fmt.Fprintf(buf, "genhelpers.RequireJmesMatch(t, jsearch, `\"%s\"[0]`, floatVal, \"%s\")\n", tagKey, fname)
		case "*int64", "int64":
			fmt.Fprintf(buf, "genhelpers.RequireJmesMatch(t, jsearch, `\"%s\"`, intVal, \"%s\")\n", tagKey, fname)
...
func (tw *TestWriter) genStructFieldTest(buf io.Writer, ins *astinspector.Inspector, pkgName, structName string, strct *ast.StructType, field *ast.Field) {
	...
	case "*string":
		...
		fmt.Fprintf(buf, `  require.Equal(t, strVal, *genStruct.%s)`+"\n", fname)
	case "string":
		// fmt.Fprintf(buf, `  require.Equal(t, strVal, %s.%s{%s: strVal}.%s)`+"\n", pkgName, structName, fname, fname)
		fmt.Fprintf(buf, `  require.Equal(t, strVal, genStruct.%s)`+"\n", fname)
	case "[]string":
		// fmt.Fprintf(buf, `  require.Contains(t, %s.%s{%s: []string{strVal}}.%s, strVal)`+"\n", pkgName, structName, fname, fname)
		fmt.Fprintf(buf, `  require.Contains(t, genStruct.%s, strVal)`+"\n", fname)

	case "*float64":
		// fmt.Fprintf(buf, `  require.Equal(t, floatVal, *%s.%s{%s: &floatVal}.%s)`+"\n", pkgName, structName, fname, fname)
		fmt.Fprintf(buf, `  require.Equal(t, floatVal, *genStruct.%s)`+"\n", fname)
	case "float64":
		// fmt.Fprintf(buf, `  require.Equal(t, floatVal, %s.%s{%s: floatVal}.%s)`+"\n", pkgName, structName, fname, fname)
		fmt.Fprintf(buf, `  require.Equal(t, floatVal, genStruct.%s)`+"\n", fname)
	case "[]float64":
		fmt.Fprintf(buf, `  require.Contains(t, genStruct.%s, floatVal)`+"\n", fname)

	case "*int64":
		// fmt.Fprintf(buf, `  require.Equal(t, intVal, *%s.%s{%s: &intVal}.%s)`+"\n", pkgName, structName, fname, fname)
		fmt.Fprintf(buf, `  require.Equal(t, intVal, *genStruct.%s)`+"\n", fname)
	case "int64":
		// fmt.Fprintf(buf, `  require.Equal(t, intVal, %s.%s{%s: intVal}.%s)`+"\n", pkgName, structName, fname, fname)
		fmt.Fprintf(buf, `  require.Equal(t, intVal, genStruct.%s)`+"\n", fname)
	...
	case "*time.Time":
		...
		fmt.Fprintf(buf, `  require.Equal(t, timeVal, *genStruct.%s)`+"\n", fname)
	case "time.Time":
		// fmt.Fprintf(buf, `  require.Equal(t, timeVal, %s.%s{%s: timeVal}.%s)`+"\n", pkgName, structName, fname, fname)
		fmt.Fprintf(buf, `  require.Equal(t, timeVal, genStruct.%s)`+"\n", fname)

	case "*bool":
		fmt.Fprintf(buf, `  require.Equal(t, trueVal, *genStruct.%s)`+"\n", fname)
	case "bool":
		fmt.Fprintf(buf, `  require.Equal(t, trueVal, genStruct.%s)`+"\n", fname)
