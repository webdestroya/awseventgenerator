...
func (tw *TestWriter) Generate() ([]byte, error) {
	...
	fmt.Fprintln(&buf, `// Code generated by awseventgenerator/internal/generators/testcode. DO NOT EDIT.`)
	fmt.Fprintln(&buf)
	fmt.Fprintln(&buf, `package testcode`)
	fmt.Fprintln(&buf)
	fmt.Fprintln(&buf, `import (`)
	fmt.Fprintln(&buf, `  "testing"`)
	fmt.Fprintln(&buf, `  "time"`)
	fmt.Fprintln(&buf, `  "encoding/json"`)
	// fmt.Fprintln(&buf, `  "github.com/jmespath/go-jmespath"`)
	fmt.Fprintln(&buf, `  "github.com/stretchr/testify/require"`)
	fmt.Fprintln(&buf)

	for k, v := range tw.folders {
		fmt.Fprintf(&buf, "  %s \"github.com/webdestroya/awseventgenerator/internal/testcode/%s\"\n", k, v)
	}

	fmt.Fprintln(&buf, `)`)
	fmt.Fprintln(&buf)
	fmt.Fprintln(&buf, `func TestGeneratedCode(t *testing.T) {`)
	fmt.Fprintln(&buf)
	...
	for tname, scopeObj := range ins.File.Scope.Objects {
		...
		case *ast.ValueSpec:
			if v.Type != nil {
				...
				// fmt.Println("DEBUG CONSTANT", tname)
...
func (tw *TestWriter) genStructMarshalTests(buf io.Writer, ins *astinspector.Inspector, pkgName, structName string, strct *ast.StructType) {
	...
	tw.genStructFakeValue(buf, ins, pkgName, structName, strct, true)
	fmt.Fprintln(buf)
	...
	for _, f := range strct.Fields.List {
		...
		switch ftypeStr {
		case "*string", "string":
			fmt.Fprintf(buf, "requireJmesMatch(t, jsearch, `\"%s\"`, strVal, \"%s.%s(%s)\")\n", tagKey, structName, fname, tagKey)
		case "*float64", "float64":
			fmt.Fprintf(buf, "requireJmesMatch(t, jsearch, `\"%s\"`, floatVal, \"%s.%s(%s)\")\n", tagKey, structName, fname, tagKey)
		case "*time.Time", "time.Time":
			fmt.Fprintf(buf, "requireJmesMatch(t, jsearch, `\"%s\"`, string(mustRet(timeVal.MarshalText())), \"%s.%s(%s)\")\n", tagKey, structName, fname, tagKey)
		default:
...
func (tw *TestWriter) genBaseFakeValue(buf io.Writer, ins *astinspector.Inspector, pkgName string, ftype ast.Node, insideArray bool) {
	...
	case *ast.ArrayType:
		...
		if inner, ok := tv.Elt.(*ast.Ident); ok {
			innerName := inner.Name
			if inner.Obj == nil {
				fmt.Fprintf(buf, `%s{`, ftypeStr)
				tw.genBaseFakeValue(buf, ins, pkgName, inner, true)
				fmt.Fprint(buf, `}`)
			} else {
				fmt.Fprintf(buf, `[]%s.%s{`, pkgName, innerName)
				tw.genBaseFakeValue(buf, ins, pkgName, inner, true)
				fmt.Fprint(buf, `}`)
				// fmt.Fprintf(buf, `%s{} /*ARRAY: inner=%s*/`, ftypeStr, innerName)
			}

		} else {
			fmt.Fprintf(buf, `%s{} /*Unknown Inner??? %T*/`, ftypeStr, tv.Elt)
		}
	...
	case *ast.MapType:
		...
		fmt.Fprint(buf, `: `)
		tw.genBaseFakeValue(buf, ins, pkgName, tv.Value, false)
		fmt.Fprintln(buf, `,`)
		fmt.Fprint(buf, `}`)
	...
	case *ast.Ident:
		...
		default:
			if tv.Obj != nil && tv.Obj.Decl != nil {
				...
				fmt.Fprintf(buf, `"Route2:%T"`, tv.Obj.Decl)
			}
		}
...
func (tw *TestWriter) genStructFieldTest(buf io.Writer, ins *astinspector.Inspector, pkgName, structName string, strct *ast.StructType, field *ast.Field) {
	...
	default:
		fmt.Fprintf(buf, `  // UNKNOWN: %s.%s.%s == %s`+"\n", pkgName, structName, fname, ftype)
		fmt.Println()
		fmt.Println("FIELD", fname, ftype)
